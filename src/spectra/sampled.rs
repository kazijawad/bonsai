...
use crate::{
    base::spectrum::{
        xyz_to_rgb, CoefficientSpectrum, SpectrumType, CIE_LAMBDA, CIE_X, CIE_Y, CIE_Y_INTEGRAL,
        CIE_Z, NUM_CIE_SAMPLES, RGB, XYZ,
    },
    utils::math::{lerp, Float},
};

const SAMPLED_LAMBDA_START: u32 = 400;
const SAMPLED_LAMBDA_END: u32 = 700;
...
static mut RGB_REFLECTANCE_WHITE_SAMPLES: SampledSpectrum =
    SampledSpectrum([0.0; SPECTRAL_SAMPLES]);
static mut RGB_REFLECTANCE_CYAN_SAMPLES: SampledSpectrum = SampledSpectrum([0.0; SPECTRAL_SAMPLES]);
static mut RGB_REFLECTANCE_MAGENTA_SAMPLES: SampledSpectrum =
    SampledSpectrum([0.0; SPECTRAL_SAMPLES]);
static mut RGB_REFLECTANCE_YELLOW_SAMPLES: SampledSpectrum =
    SampledSpectrum([0.0; SPECTRAL_SAMPLES]);
static mut RGB_REFLECTANCE_RED_SAMPLES: SampledSpectrum = SampledSpectrum([0.0; SPECTRAL_SAMPLES]);
static mut RGB_REFLECTANCE_GREEN_SAMPLES: SampledSpectrum =
    SampledSpectrum([0.0; SPECTRAL_SAMPLES]);
static mut RGB_REFLECTANCE_BLUE_SAMPLES: SampledSpectrum = SampledSpectrum([0.0; SPECTRAL_SAMPLES]);

static mut RGB_ILLUMINANT_WHITE_SAMPLES: SampledSpectrum = SampledSpectrum([0.0; SPECTRAL_SAMPLES]);
static mut RGB_ILLUMINANT_CYAN_SAMPLES: SampledSpectrum = SampledSpectrum([0.0; SPECTRAL_SAMPLES]);
static mut RGB_ILLUMINANT_MAGENTA_SAMPLES: SampledSpectrum =
    SampledSpectrum([0.0; SPECTRAL_SAMPLES]);
static mut RGB_ILLUMINANT_YELLOW_SAMPLES: SampledSpectrum =
    SampledSpectrum([0.0; SPECTRAL_SAMPLES]);
static mut RGB_ILLUMINANT_RED_SAMPLES: SampledSpectrum = SampledSpectrum([0.0; SPECTRAL_SAMPLES]);
static mut RGB_ILLUMINANT_GREEN_SAMPLES: SampledSpectrum = SampledSpectrum([0.0; SPECTRAL_SAMPLES]);
static mut RGB_ILLUMINANT_BLUE_SAMPLES: SampledSpectrum = SampledSpectrum([0.0; SPECTRAL_SAMPLES]);
...
impl CoefficientSpectrum for SampledSpectrum {
    ...
    // Must be called on application load before
    // all other references of SampledSpectrum.
    fn init() {
        unsafe {
            // Compute XYZ matching functions.
            for i in 0..Self::NUM_SAMPLES {
                let wl0 = lerp(
                    (i as Float) / (Self::NUM_SAMPLES as Float),
                    SAMPLED_LAMBDA_START as Float,
                    SAMPLED_LAMBDA_END as Float,
                );
                let wl1 = lerp(
                    ((i + 1) as Float) / (Self::NUM_SAMPLES as Float),
                    SAMPLED_LAMBDA_START as Float,
                    SAMPLED_LAMBDA_END as Float,
                );

                X[i] = Self::average_samples(&CIE_LAMBDA, &CIE_X, NUM_CIE_SAMPLES, wl0, wl1);
                Y[i] = Self::average_samples(&CIE_LAMBDA, &CIE_Y, NUM_CIE_SAMPLES, wl0, wl1);
                Z[i] = Self::average_samples(&CIE_LAMBDA, &CIE_Z, NUM_CIE_SAMPLES, wl0, wl1);
            }
            ...
            for i in 0..Self::NUM_SAMPLES {
                ...
                RGB_REFLECTANCE_WHITE_SAMPLES[i] = Self::average_samples(
                    &RGB_SPECTRAL_LAMBDA,
                    &RGB_REFLECTANCE_WHITE,
                    RGB_SPECTRAL_SAMPLES,
                    wl0,
                    wl1,
                );
                RGB_REFLECTANCE_CYAN_SAMPLES[i] = Self::average_samples(
                    &RGB_SPECTRAL_LAMBDA,
                    &RGB_REFLECTANCE_CYAN,
                    RGB_SPECTRAL_SAMPLES,
                    wl0,
                    wl1,
                );
                RGB_REFLECTANCE_MAGENTA_SAMPLES[i] = Self::average_samples(
                    &RGB_SPECTRAL_LAMBDA,
                    &RGB_REFLECTANCE_MAGENTA,
                    RGB_SPECTRAL_SAMPLES,
                    wl0,
                    wl1,
                );
                RGB_REFLECTANCE_YELLOW_SAMPLES[i] = Self::average_samples(
                    &RGB_SPECTRAL_LAMBDA,
                    &RGB_REFLECTANCE_YELLOW,
                    RGB_SPECTRAL_SAMPLES,
                    wl0,
                    wl1,
                );
                RGB_REFLECTANCE_RED_SAMPLES[i] = Self::average_samples(
                    &RGB_SPECTRAL_LAMBDA,
                    &RGB_REFLECTANCE_RED,
                    RGB_SPECTRAL_SAMPLES,
                    wl0,
                ...
                RGB_REFLECTANCE_GREEN_SAMPLES[i] = Self::average_samples(
                    &RGB_SPECTRAL_LAMBDA,
                    &RGB_REFLECTANCE_GREEN,
                    RGB_SPECTRAL_SAMPLES,
                    wl0,
                    wl1,
                );
                RGB_REFLECTANCE_BLUE_SAMPLES[i] = Self::average_samples(
                    &RGB_SPECTRAL_LAMBDA,
                    &RGB_REFLECTANCE_BLUE,
                    RGB_SPECTRAL_SAMPLES,
                    wl0,
                    wl1,
                );

                RGB_ILLUMINANT_WHITE_SAMPLES[i] = Self::average_samples(
                    &RGB_SPECTRAL_LAMBDA,
                    &RGB_ILLUMINANT_WHITE,
                    RGB_SPECTRAL_SAMPLES,
                    wl0,
                    wl1,
                );
                RGB_ILLUMINANT_CYAN_SAMPLES[i] = Self::average_samples(
                    &RGB_SPECTRAL_LAMBDA,
                    &RGB_ILLUMINANT_CYAN,
                    RGB_SPECTRAL_SAMPLES,
                    wl0,
                ...
                RGB_ILLUMINANT_MAGENTA_SAMPLES[i] = Self::average_samples(
                    &RGB_SPECTRAL_LAMBDA,
                    &RGB_ILLUMINANT_MAGENTA,
                    RGB_SPECTRAL_SAMPLES,
                    wl0,
                    wl1,
                );
                RGB_ILLUMINANT_YELLOW_SAMPLES[i] = Self::average_samples(
                    &RGB_SPECTRAL_LAMBDA,
                    &RGB_ILLUMINANT_YELLOW,
                    RGB_SPECTRAL_SAMPLES,
                    wl0,
                    wl1,
                );
                RGB_ILLUMINANT_RED_SAMPLES[i] = Self::average_samples(
                    &RGB_SPECTRAL_LAMBDA,
                    &RGB_ILLUMINANT_RED,
                    RGB_SPECTRAL_SAMPLES,
                    wl0,
                ...
                RGB_ILLUMINANT_GREEN_SAMPLES[i] = Self::average_samples(
                    &RGB_SPECTRAL_LAMBDA,
                    &RGB_ILLUMINANT_GREEN,
                    RGB_SPECTRAL_SAMPLES,
                    wl0,
                    wl1,
                );
                RGB_ILLUMINANT_BLUE_SAMPLES[i] = Self::average_samples(
                    &RGB_SPECTRAL_LAMBDA,
                    &RGB_ILLUMINANT_BLUE,
                    RGB_SPECTRAL_SAMPLES,
                    wl0,
    ...
    fn from_sampled(lambda: &mut [Float], values: &mut [Float], n: usize) -> Self {
        ...
        for i in 0..Self::NUM_SAMPLES {
            ...
            spectrum[i] = Self::average_samples(lambda, values, n, lambda0, lambda1);
        }
    ...
    fn from_xyz(xyz: &XYZ, spectrum_type: SpectrumType) -> Self {
        ...
        Self::from_rgb(&rgb, spectrum_type)
    }

    fn from_rgb(rgb: &RGB, spectrum_type: SpectrumType) -> Self {
        let mut spectrum = SampledSpectrum::default();

        if let SpectrumType::Reflectance = spectrum_type {
            // Convert reflectance spectrum to RGB.
            if rgb[0] <= rgb[1] && rgb[0] <= rgb[2] {
                // Compute reflectance with rgb[0] as minimum.
                unsafe {
                    spectrum += RGB_REFLECTANCE_WHITE_SAMPLES * rgb[0];
                    if rgb[1] <= rgb[2] {
                        spectrum += RGB_REFLECTANCE_CYAN_SAMPLES * (rgb[1] - rgb[0]);
                        spectrum += RGB_REFLECTANCE_BLUE_SAMPLES * (rgb[2] - rgb[1]);
                    } else {
                        spectrum += RGB_REFLECTANCE_CYAN_SAMPLES * (rgb[2] - rgb[0]);
                        spectrum += RGB_REFLECTANCE_GREEN_SAMPLES * (rgb[1] - rgb[2]);
                    }
                }
            } else if rgb[1] <= rgb[0] && rgb[1] <= rgb[2] {
                // Compute reflectance with rgb[1] as minimum.
                unsafe {
                    spectrum += RGB_REFLECTANCE_WHITE_SAMPLES * rgb[1];
                    if rgb[0] <= rgb[2] {
                        spectrum += RGB_REFLECTANCE_CYAN_SAMPLES * (rgb[0] - rgb[1]);
                        spectrum += RGB_REFLECTANCE_BLUE_SAMPLES * (rgb[2] - rgb[0]);
                    } else {
                        spectrum += RGB_REFLECTANCE_CYAN_SAMPLES * (rgb[2] - rgb[1]);
                        spectrum += RGB_REFLECTANCE_GREEN_SAMPLES * (rgb[0] - rgb[2]);
                    }
                }
            } else {
                // Compute reflectance with rgb[2] as minimum.
                unsafe {
                    spectrum += RGB_REFLECTANCE_WHITE_SAMPLES * rgb[2];
                    if rgb[0] <= rgb[2] {
                        spectrum += RGB_REFLECTANCE_CYAN_SAMPLES * (rgb[0] - rgb[2]);
                        spectrum += RGB_REFLECTANCE_BLUE_SAMPLES * (rgb[1] - rgb[0]);
                    } else {
                        spectrum += RGB_REFLECTANCE_CYAN_SAMPLES * (rgb[1] - rgb[2]);
                        spectrum += RGB_REFLECTANCE_GREEN_SAMPLES * (rgb[0] - rgb[1]);
                    }
                }
            }
            ...
            if rgb[0] <= rgb[1] && rgb[0] <= rgb[2] {
                // Compute reflectance with rgb[0] as minimum.
                unsafe {
                    spectrum += RGB_ILLUMINANT_WHITE_SAMPLES * rgb[0];
                    if rgb[1] <= rgb[2] {
                        spectrum += RGB_ILLUMINANT_CYAN_SAMPLES * (rgb[1] - rgb[0]);
                        spectrum += RGB_ILLUMINANT_BLUE_SAMPLES * (rgb[2] - rgb[1]);
                    } else {
                        spectrum += RGB_ILLUMINANT_CYAN_SAMPLES * (rgb[2] - rgb[0]);
                        spectrum += RGB_ILLUMINANT_GREEN_SAMPLES * (rgb[1] - rgb[2]);
                    }
                }
            } else if rgb[1] <= rgb[0] && rgb[1] <= rgb[2] {
                // Compute reflectance with rgb[1] as minimum.
                unsafe {
                    spectrum += RGB_ILLUMINANT_WHITE_SAMPLES * rgb[1];
                    if rgb[0] <= rgb[2] {
                        spectrum += RGB_ILLUMINANT_CYAN_SAMPLES * (rgb[0] - rgb[1]);
                        spectrum += RGB_ILLUMINANT_BLUE_SAMPLES * (rgb[2] - rgb[0]);
                    } else {
                        spectrum += RGB_ILLUMINANT_CYAN_SAMPLES * (rgb[2] - rgb[1]);
                        spectrum += RGB_ILLUMINANT_GREEN_SAMPLES * (rgb[0] - rgb[2]);
                    }
                }
            } else {
                // Compute reflectance with rgb[2] as minimum.
                unsafe {
                    spectrum += RGB_ILLUMINANT_WHITE_SAMPLES * rgb[2];
                    if rgb[0] <= rgb[2] {
                        spectrum += RGB_ILLUMINANT_CYAN_SAMPLES * (rgb[0] - rgb[2]);
                        spectrum += RGB_ILLUMINANT_BLUE_SAMPLES * (rgb[1] - rgb[0]);
                    } else {
                        spectrum += RGB_ILLUMINANT_CYAN_SAMPLES * (rgb[1] - rgb[2]);
                        spectrum += RGB_ILLUMINANT_GREEN_SAMPLES * (rgb[0] - rgb[1]);
                    }
                }
            }
    ...
    fn to_xyz(&self, xyz: &mut XYZ) {
        ...
        let scale = ((SAMPLED_LAMBDA_END - SAMPLED_LAMBDA_START) as Float)
            / (CIE_Y_INTEGRAL * (Self::NUM_SAMPLES as Float));
        xyz[0] *= scale;
        xyz[1] *= scale;
        xyz[2] *= scale;
    }
...
impl ops::SubAssign for SampledSpectrum {
...
impl ops::MulAssign for SampledSpectrum {
...
impl ops::DivAssign for SampledSpectrum {
...
const RGB_SPECTRAL_LAMBDA: [Float; RGB_SPECTRAL_SAMPLES] = [
...
const RGB_REFLECTANCE_WHITE: [Float; RGB_SPECTRAL_SAMPLES] = [
...
const RGB_REFLECTANCE_CYAN: [Float; RGB_SPECTRAL_SAMPLES] = [
...
const RGB_REFLECTANCE_MAGENTA: [Float; RGB_SPECTRAL_SAMPLES] = [
...
const RGB_REFLECTANCE_YELLOW: [Float; RGB_SPECTRAL_SAMPLES] = [
...
const RGB_REFLECTANCE_RED: [Float; RGB_SPECTRAL_SAMPLES] = [
...
const RGB_REFLECTANCE_GREEN: [Float; RGB_SPECTRAL_SAMPLES] = [
...
const RGB_REFLECTANCE_BLUE: [Float; RGB_SPECTRAL_SAMPLES] = [
...
const RGB_ILLUMINANT_WHITE: [Float; RGB_SPECTRAL_SAMPLES] = [
...
const RGB_ILLUMINANT_CYAN: [Float; RGB_SPECTRAL_SAMPLES] = [
...
const RGB_ILLUMINANT_MAGENTA: [Float; RGB_SPECTRAL_SAMPLES] = [
...
const RGB_ILLUMINANT_YELLOW: [Float; RGB_SPECTRAL_SAMPLES] = [
...
const RGB_ILLUMINANT_RED: [Float; RGB_SPECTRAL_SAMPLES] = [
...
const RGB_ILLUMINANT_GREEN: [Float; RGB_SPECTRAL_SAMPLES] = [
...
const RGB_ILLUMINANT_BLUE: [Float; RGB_SPECTRAL_SAMPLES] = [
