...
cfg_if::cfg_if! {
    if #[cfg(feature = "sampled-spectrum")] {
        pub type Spectrum = crate::spectra::sampled::SampledSpectrum;
    } else {
        pub type Spectrum = crate::spectra::rgb::RGBSpectrum;
    }
}
    ...
    + ops::IndexMut<usize>
{
    ...
    fn is_samples_sorted(lambda: &[Float], n: usize) -> bool {
        for i in 0..(n - 1) {
    ...
    fn sort_samples(lambda: &mut [Float], values: &mut [Float], n: usize) {
        let mut sorted: Vec<(Float, Float)> = Vec::with_capacity(n);
        for i in 0..n {
            sorted.push((lambda[i], values[i]))
        }
    ...
    fn average_samples(
        ...
        for i in 0..(n - 1) {
            debug_assert!(lambda[i + 1] > lambda[i]);
        }
        debug_assert!(lambda_start < lambda_end);
        ...
        // Add contributions of constant segments before and after samples.
        let mut sum: Float = 0.0;
        if lambda_start < lambda[0] {
            sum += values[0] * (lambda[0] - lambda_start);
        }
        if lambda_end > lambda[n - 1] {
            sum += values[n - 1] * (lambda_end - lambda[n - 1]);
        }
        ...
        let interpolate = |i: usize, w: Float| -> Float {
            lerp(
        ...
        while i + 1 < n && lambda_end >= lambda[i] {
            let lambda_start_segment = lambda_start.max(lambda[i]);
            let lambda_end_segment = lambda_end.min(lambda[i]);

            sum += 0.5
                * (interpolate(i, lambda_start_segment) + interpolate(i, lambda_end_segment))
                * (lambda_end_segment - lambda_start_segment);
    ...
    fn interpolate_samples(lambda: &[Float], values: &[Float], n: usize, l: Float) -> Float {
        for i in 0..(n - 1) {
            debug_assert!(lambda[i + 1] > lambda[i]);
        }
        ...
        let offset = find_interval(n, |i| lambda[i] <= l);
        debug_assert!(l >= lambda[offset] && l <= lambda[offset + 1]);
        let t = (l - lambda[offset]) / (lambda[offset + 1] - lambda[offset]);

        lerp(t, values[offset], values[offset + 1])
    }

    fn blackbody(lambda: &[Float], n: usize, t: Float, le: &mut [Float]) {
        if t <= 0.0 {
            for i in 0..n {
                le[i] = 0.0;
            }
        ...
        for i in 0..n {
            ...
            le[i] = (2.0 * h * c * c) / (lambda5 * ((h * c) / (l * kb * t)) - 1.0).exp();
            debug_assert!(!le[i].is_nan());
        }
    }

    fn blackbody_normalized(lambda: &[Float], n: usize, t: Float, le: &mut [Float]) {
        Self::blackbody(lambda, n, t, le);

        // Normalize values based on maximum blackbody radiance.
        let lambda_max = vec![2.8977721e-3 / t * 1e9];
        let mut max_l = vec![0.0];
        Self::blackbody(&lambda_max, 1, t, &mut max_l);
...
pub const CIE_X: [Float; NUM_CIE_SAMPLES] = [
    0.0001299000,
    0.0001458470,
    0.0001638021,
    0.0001840037,
